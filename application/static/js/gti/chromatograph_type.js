function add_type() {
    ...
    url = 'http://' + document.location.host + '/api/gti/chromatograph_type'
    xhr.open("POST", url);
    xhr.send(formData);
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
            if(xhr.status == 200) {
                ...
                link.setAttribute('href', '/gti/directory/chromatograph_type/' + type['id'])
...
function edit_type() {
    ...
    var xhr = new XMLHttpRequest();
    url = 'http://' + document.location.host + '/api/gti/chromatograph_type/' + type_id
    xhr.open("PUT", url);
    xhr.send(formData);
...
function delete_type() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
            if(xhr.status == 204) {
                window.location.replace('http://' + document.location.host + '/gti/directory/chromatograph_type')
            }
            if(xhr.status == 400 || xhr.status == 500) {
                header.innerHTML = 'Провал'
            }
